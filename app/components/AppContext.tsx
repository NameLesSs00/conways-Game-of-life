...
import { useState, createContext, ReactNode, useContext, useRef, useEffect } from "react";
...
export function AppContext({ children }: { children: ReactNode }) {
  ...
  // Reset grid to all dead cells and pause simulation
  const clearGrid = () => {
  ...
  const setGridPattern = (patternId: number) => {
    ...
    if (patternId === 1) { 
        // Gosper Glider Gun Pattern (already provided)
        const gliderGun = [
            [5, 1], [5, 2], [6, 1], [6, 2],
            [5, 11], [6, 11], [7, 11],
            [4, 12], [8, 12],
      ...
      largeSpaceship.forEach(([r, c]) => {
          if (r + offsetRow >= 0 && r + offsetRow < Rows && c + offsetCol >= 0 && c + offsetCol < Cols) {
        ...
        for (let c = 0; c < Cols; c++) {
          ...
          for (let dr = -1; dr <= 1; dr++) {
            for (let dc = -1; dc <= 1; dc++) {
              ...
              // Boundary check - cells outside die (are treated as dead)
  ...
  // Toggle a cell's state (alive or dead)
  ...
  // start the game with a random set
  ...
  useEffect(() => {
    ...
    setGrid(initialGrid);
    resume(); // Start simulation immediately
  }, []);
